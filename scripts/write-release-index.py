...
def build_index(version, architecture, profiles, capabilities, packages, selected, filenames):
    if architecture not in ("amd64", "arm64"):
        raise ValueError(f"unsupported architecture {architecture!r} (want amd64|arm64)")
    ...
    for package, entry in packages.items():
        ...
        for capability in caps:
            ...
            if capability == "inference":
                ...
                if not isinstance(runtime, dict) or not identifier(runtime.get("inferenceEngine")):
                    raise ValueError(f"package {package!r} needs runtime inferenceEngine")
                if "architecture" in runtime:
                    raise ValueError(
                        f"package {package!r} must not set runtime.architecture "
                        "(architecture is product-level TARGET_ARCH)"
                    )
...
def render_index(index):
    ...
    lines = [f"version: {index['version']}", f"architecture: {index['architecture']}", "packs:"]
