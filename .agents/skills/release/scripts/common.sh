...
reject_removed_build_publish_packaging_keys() {
  ...
  if [[ -n "$(config_get_optional "${config_path}" "build_flow.appliance_packs_env" || true)" ]]; then
    fail "build_flow.appliance_packs_env was removed; set build_flow.appliance_packs to a literal value (all|foundation|foundation,storage-network,build-workflows|foundation,inference|foundation,video)"
  fi
