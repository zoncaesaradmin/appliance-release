...
resolve_host_mdns_enabled() {
  local config_path="$1"
  local host_mdns_enabled="${2:-}"
  local normalized=""
  if [[ -z "${host_mdns_enabled}" ]]; then
    host_mdns_enabled="$(config_get_optional "${config_path}" "install.host_mdns_enabled" || true)"
  fi
  if [[ -z "${host_mdns_enabled}" ]]; then
    printf 'false\n'
    return 0
  fi
  normalized="$(printf '%s' "${host_mdns_enabled}" | tr '[:upper:]' '[:lower:]')"
  case "${normalized}" in
    1|true|yes|on)
      printf 'true\n'
      ;;
    0|false|no|off)
      ...
      ;;
    *)
      fail "install.host_mdns_enabled must be true or false"
      ;;
  esac
}
...
# When wifi-ap is enabled, read install.host_wifi_ap_psk_env (name of an env
# var on the orchestrator) and return that variable's value. Never store the
# PSK in yaml. Prints empty and returns 0 when wifi-ap is disabled.
resolve_host_wifi_ap_psk() {
  local config_path="$1"
  local host_wifi_ap_enabled="$2"
  local psk_env=""
  local psk=""
  if ! bool_true "${host_wifi_ap_enabled}"; then
    printf '\n'
    return 0
  fi
  psk_env="$(config_get_optional "${config_path}" "install.host_wifi_ap_psk_env" || true)"
  psk_env="$(printf '%s' "${psk_env}" | tr -d '[:space:]')"
  [[ -n "${psk_env}" ]] || fail "install.host_wifi_ap_enabled=true requires install.host_wifi_ap_psk_env (env var name for the WPA2 PSK, e.g. HOST_WIFI_AP_PSK)"
  psk="$(resolve_env_value "${psk_env}" "install.host_wifi_ap_psk_env")"
  psk="$(printf '%s' "${psk}" | tr -d '\r\n')"
  [[ -n "${psk}" ]] || fail "install.host_wifi_ap_psk_env ${psk_env} resolved empty"
  printf '%s' "${psk}"
}
