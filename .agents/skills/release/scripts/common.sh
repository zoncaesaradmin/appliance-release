...
# Namespace:  appliance-ctl cmd/zonctl/main.go defaultChartNamespace = "ace-system"
# Deployment: appliance-code deploy/charts/appliance-control-plane:
#   templates/_helpers.tpl nameOverride|default "controlplane"
...
PRODUCT_CONTROL_PLANE_NAMESPACE="ace-system"
PRODUCT_CONTROL_PLANE_DEPLOYMENT="controlplane"
...
reject_removed_build_publish_packaging_keys() {
  ...
  if [[ -n "$(config_get_optional "${config_path}" "build_flow.appliance_packs_env" || true)" ]]; then
    fail "build_flow.appliance_packs_env was removed; set build_flow.appliance_packs to a literal value (all|foundation|foundation,dev-platform|foundation,std-llm|foundation,std-llm,open-webui|foundation,acc-llm)"
  fi
