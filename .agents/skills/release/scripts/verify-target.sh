...
if [[ -n "${BUNDLE_BIN_DIR}" && "${ARTIFACT_READINESS_CMD}" == "sudo kubectl -n registry wait --for=condition=Available deployment/artifact-registry --timeout=120s && sudo kubectl -n registry get pvc appliance-registry-data" ]]; then
  ARTIFACT_READINESS_CMD="sudo env PATH=${BUNDLE_BIN_DIR}:${DEFAULT_TARGET_PATH} kubectl -n registry wait --for=condition=Available deployment/artifact-registry --timeout=120s && env PATH=${BUNDLE_BIN_DIR}:${DEFAULT_TARGET_PATH} kubectl -n registry get pvc appliance-registry-data"
fi
