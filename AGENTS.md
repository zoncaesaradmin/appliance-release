...
- There are exactly **two** build-host modes — not a mix per dependency:
  ...
  - **Offline:** every such input is fetched only from the LAN Artifact Server (`DEV_REGISTRY` OCI + files API) after `TARGET_ARCH=amd64 make seed-build-deps` (or equivalent). Misses fail closed; no public upstream fallback.
...
- **Whenever you add a new third-party / upstream image or file input to packaging** (export script, `build-full-bundle`, release-input, chart wrap, etc.), you **must** also:
  1. Add or extend a `deps/<name>/` seed package (`pins.env`, `scripts/build.sh`, `scripts/push.sh`, Makefile, README) so `TARGET_ARCH=amd64 make seed-build-deps` publishes it to LAN `build-cache` / files API.
