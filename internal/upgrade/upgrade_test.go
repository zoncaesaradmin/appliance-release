...
// Online mode: the upgrade sequence is identical to the offline path once
// install.Source.Resolve has handed back local artifact paths, so a
// successful online upgrade proves that reuse works end to end (fetched
// K3s binary/CRDs/config, chart pulled via `helm pull` from an OCI
// registry, no image preload).
func TestUpgrade_OnlineSource_Success(t *testing.T) {
	env := setupEnvironment(t, "2.3.0", "v1.30.4+k3s1", "3.5.1", "2.3.0")

	manifest := install.PlatformManifest{
		PlatformVersion:         "2.4.0",
		ReleaseID:               "release-2.4.0",
		K3sVersion:              "v1.30.4+k3s1", // unchanged from installed: no binary swap
	...
	// pullRun is OnlineSource's own helm runner (`helm pull`), separate
	// from the orchestrator's HelmRun (CRD apply / chart install), since
	// only the former needs to fake writing a chart tarball into -d.
	pullRun := func(_ context.Context, name string, args ...string) (string, error) {
		var destDir string
		for i, a := range args {
			if a == "-d" && i+1 < len(args) {
				destDir = args[i+1]
		...
		if destDir == "" {
			return "", fmt.Errorf("no -d destination given")
		}
	...
	onlineSource := install.OnlineSource{Manifest: manifest, Get: get, HelmRun: pullRun, WorkDir: t.TempDir()}
	updated, checks, err := orch.Upgrade(context.Background(), onlineSource, env.options("2.4.0"))
	if err != nil {
		t.Fatalf("expected online upgrade to succeed, got: %v", err)
	}
	if updated.InstalledVersion != "2.4.0" || updated.InstalledReleaseID != manifest.ReleaseID {
		t.Errorf("unexpected result: %+v", updated)
	}
	for _, call := range fake.calls {
		if call == "write-config" || call == "write-unit" || call == "install-binary" {
			t.Errorf("expected no k3s binary/config/unit mutation when the k3s version is unchanged, got %v", fake.calls)
			break
		}
	}

	foundBinaryCheck := false
	for _, c := range checks {
		if c.ID == "upgrade-k3s-binary" && strings.Contains(c.Message, "unchanged") {
			foundBinaryCheck = true
		}
	}
	if !foundBinaryCheck {
		t.Error("expected an evidence check noting the k3s binary was left unchanged")
	}
}

// Tamper: a digest mismatch on a fetched artifact must fail the online
// upgrade closed, before any k3s/backup mutation happens.
func TestUpgrade_OnlineSource_DigestMismatchFailsClosed(t *testing.T) {
	env := setupEnvironment(t, "2.3.0", "v1.30.4+k3s1", "3.5.1", "2.3.0")
	...
	manifest := install.PlatformManifest{
		...
		K3sBinaryURL:            "https://example.invalid/k3s",
		K3sBinaryDigest:         digestOf("expected bytes"),
		ChartRepository:         "oci://registry.example.invalid/zon-platform",
		ChartName:               "zon",
		ChartVersion:            "2.4.0",
		CRDsURL:                 "https://example.invalid/crds.yaml",
	...
	get := func(_ context.Context, url string) ([]byte, error) {
		if url == manifest.K3sBinaryURL {
			return []byte("tampered bytes, not what was pinned"), nil
		}
		return nil, fmt.Errorf("unexpected URL: %s", url)
	}
	...
	onlineSource := install.OnlineSource{Manifest: manifest, Get: get, HelmRun: fcli.Run, WorkDir: t.TempDir()}
	_, _, err := orch.Upgrade(context.Background(), onlineSource, env.options("2.4.0"))
	if err == nil {
		t.Fatal("expected a k3s binary digest mismatch to fail the online upgrade")
	}
	if len(fake.calls) != 0 {
		t.Errorf("expected no k3s mutation before artifact resolution succeeds, got %v", fake.calls)
	}
}
