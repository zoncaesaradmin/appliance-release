...
| `open-webui` | files API `build-deps/open-webui/<locked-commit>/open-webui-source.tar.gz` | Verified upstream source for the patched optional Web UI in `std-llm` / `acc-llm`; never foundation |
